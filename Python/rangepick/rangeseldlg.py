...
class Rangeseldlg(QDialog, ui_rangeseldlg.Ui_rangeseldlg):
    ...
    def copyin_ranges(self):
        ...
        maxminx, maxminy = self.specdata.getmaxmin()
        ...
        self.plotter = plotter.Plotter(self.gpopts)
        
        for n,id in enumerate(self.specdata.obslist()):
            jd = "%.4f" % id.modjdate
    ...
    def updateplot(self):
        ...
        try:
            ...
            for n,pf in enumerate(selected):
                tf = os.path.join(self.tempdir, "tf%d" % n)
                self.specdata[pf].write_datafile(tf)
                plotfiles.append(tf)
