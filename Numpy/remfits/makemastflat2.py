...
for file, descr in zip(files, descrs):
    ...
    tfdat = trimarrays.trimzeros(fdat)
    tfdat, = rg.apply_trims(None, tfdat)
    tbias, = trimarrays.trimto(tfdat, biasdata)
    tfdat = tfdat - tbias
    fdat -= biasdata
    fdat /= tfdat.mean()
    ims.append(fdat)
...
final_image = ss.gmean(ims, axis=0)
...
first_header.set('FILENAME', miscutils.removesuffix(outfile, all=True), ' filename of this fianl image')
