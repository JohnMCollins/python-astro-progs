...
parsearg.add_argument('--title', type=str, default='Fit of std devs versus meen', help='Title for plot')
...
dbcurs.execute("SELECT mean,std,minv,maxv FROM iforbinf WHERE " + " AND ".join(fieldselect))
...
if maxval is None:
    if bymaxmin:
        for m in maxrange:
            ...
            errs.append(lrstd)
        if xlab is None:
            xlab = "Cutoff value - maximum value with minimum at %d" % minval
    else:
        for m in maxrange:
            ...
            errs.append(lrstd)
        if xlab is None:
            xlab = "Cutoff value - mean value with minimum at %d" % minval
    xax = maxrange
else:
    if bymaxmin:
        for m in minrange:
            ...
            errs.append(lrstd)
        if xlab is None:
            xlab = "Cutoff value - minimum value with maximum at %d" % maxval
    else:
        for m in minrange:
            ...
            errs.append(lrstd)
        if xlab is None:
            xlab = "Cutoff value - mean value with maximum at %d" % maxval
    xax = minrange
