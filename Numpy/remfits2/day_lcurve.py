...
class Result:
    ...
    def __init__(self, when, filtname, findresult, obsind):
        self.when = when
...
parsearg.add_argument('--title', type=str, default='Display of ADU count for {name:s}', help='Title for plat, use {name=s} to insert object name')
...
parsearg.add_argument('--ylabel', type=str, default='ADU count * {yscale:.6g}', help='Y axis label')
...
if filt and verbose:
    for f in filt:
        if nresults[f] < 2:
            print("Insufficient results for filter", f, file=sys.stderr)
