...
parsearg.add_argument('--refobjs', type=str, nargs='+', required=True, help='Reference objects')
...
for ro in refobjs:
    try:
        newrefobjs.append(objdata.get_objname(mycurs, ro))
    except objdata.ObjDataError as e:
        print("Trouble with ref obj", ro, e.args[0], file=sys.stderr)

if len(newrefobjs) != len(refobjs):
    print("Aborting due to ref obj errors", file=sys.stderr)
    sys.exit(11)
...
for ffile in flist:
    ...
    for ro in refobjs:
        try:
            ...
            errors += 1
    if errors != 0 or totro <= 0.0:
        continue

    datelist[rstr.filter].append(rstr.obsdate)
    aducount[rstr.filter].append(trstr.adus / totro)
